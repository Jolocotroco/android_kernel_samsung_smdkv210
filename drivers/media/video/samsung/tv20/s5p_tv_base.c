...
bool cam_start;
bool g_cam_enable = false;
void force_hdmi_show_img(bool state);
static bool bIsEarlySuspend;
...
void s5p_tv_base_handle_cable(void)
{
	...
	if (bIsEarlySuspend == true)
	{
		s5p_hdmi_video_set_bluescreen(true,
		g_s5ptv_status.hdmi_video_blue_screen.cb_b,
		g_s5ptv_status.hdmi_video_blue_screen.y_g,
		g_s5ptv_status.hdmi_video_blue_screen.cr_r);
	}
#endif
}
...
#ifdef CONFIG_PM
#ifdef CONFIG_HAS_EARLYSUSPEND
int s5p_tv_early_suspend(struct early_suspend *h)
{

	struct s5p_tv_status *st = &g_s5ptv_status;
	
	s5p_hdmi_video_set_bluescreen(true,
		st->hdmi_video_blue_screen.cb_b,
		st->hdmi_video_blue_screen.y_g,
		st->hdmi_video_blue_screen.cr_r);

	bIsEarlySuspend = true;
	
	return 0;
}

int s5p_tv_late_resume(struct early_suspend *h)
{

	struct s5p_tv_status *st = &g_s5ptv_status;
	
	s5p_hdmi_video_set_bluescreen(false,
		st->hdmi_video_blue_screen.cb_b,
		st->hdmi_video_blue_screen.y_g,
		st->hdmi_video_blue_screen.cr_r);

	bIsEarlySuspend = false;
