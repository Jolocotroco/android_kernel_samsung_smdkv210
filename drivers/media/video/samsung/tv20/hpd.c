...
// USE GPH1:5 bit as hdmi detect PIN
#define HDMI_IRQ_NUM    IRQ_EINT13
#define HDMI_DET_PIN    S5PV210_GPH1(5)
#define DET_PIN_SFUNC   S3C_GPIO_SFN(0x4)
#define DET_PIN_SFUNC_B S3C_GPIO_SFN(0xf)
#define DET_PIN_TYPE    S3C_GPIO_PULL_DOWN
#define DET_PIN_GROUP   S5PV210_GPH1DRV
...
irqreturn_t s5p_hpd_irq_handler(int irq, void *dev_id)
{
    ...
    {
		last_flag = s5p_hdmi_get_interrupts();
		if (!(last_flag & (1 << HDMI_IRQ_HPD_PLUG | 1 << HDMI_IRQ_HPD_UNPLUG))) 
		{
            ret = IRQ_NONE;
		...
		if ((irq == HDMI_IRQ_HPD_UNPLUG) || (irq == HDMI_IRQ_HPD_PLUG))
		{
            if (s5p_hdmi_get_hpd_status())
            {
                    s5p_hdmi_clear_pending(HDMI_IRQ_HPD_PLUG);
    ...
    mod_timer(&hpd_irq_check_timer, jiffies + 1 * HZ);
	spin_unlock_irqrestore(&g_hpd_struct.lock,flag);
...
static void hpd_irq_timer_expired_func(void)
{
    int irq = IRQ_EINT13;

    printk("hpd_irq_timer_expired_func\n");
    if (atomic_read(&g_hdmi_status)) 
    {
        /* HDMI on */
        irq_hdmi(irq);
        HPDIFPRINTK("HDMI HPD interrupt\n");
        printk("HDMI HPD interrupt\n");
    } 
    else 
    {
        /* HDMI off */
        irq_eint(irq);
        HPDIFPRINTK("EINT HPD interrupt\n");
        printk("EINT HPD interrupt\n");
    }
}

static void hpd_irq_check_timer_func(unsigned long dummy)
{
    hpd_irq_timer_expired_func();
}
