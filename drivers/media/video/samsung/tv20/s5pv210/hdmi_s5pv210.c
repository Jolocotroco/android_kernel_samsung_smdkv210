...
int s5p_hdmi_phy_tx_power(bool on, bool bWaitComplete)
{
	u32 size;
	u8 *buffer;
	u8 read_buffer[0x40] = {0, };
	...
	HDMIPRINTK("%s, orig reg: [%#x], turn to %s\n", __func__, read_buffer[0x17], on? "on" : "off");
	...
	// if request to wait above action complete
	if(bWaitComplete)
	{
		...
		do {
			/* read data */
			if (i2c_hdmi_phy_read(PHY_I2C_ADDRESS, size, read_buffer) != 0) {
				printk("i2c_hdmi_phy_read failed...\n");
				return EINVAL;
			}

			HDMIPRINTK("%s #%-3d, reg: [%#x], turn to %s\n", __func__, time_out, read_buffer[0x17], on ? "on" : "off");
			if(on)
			{
				bComplete = ((read_buffer[0x17] & (0x1 << 1)) == (0x0));
			}
			else
			{
				bComplete = ((read_buffer[0x17] & (0x1 << 1)) == (0x1 << 1));
			}

			time_out--;
			msleep(10);
		} while (!bComplete && (time_out > 0));
	}
	HDMIPRINTK("-%s\n", __func__);
...
*/
	...
	if (result)
        {
		s5p_hdmi_phy_tx_power(false, true);
		msleep(100);
