...
void s5p_vp_set_field_id(enum s5p_vp_field mode)
{
	...
	VPPRINTK("0x%08x\n", readl(g_vp_base + S5P_VP_FIELD_ID));
}
						...
						u32 bottom_c_addr)
{
	...
	VPPRINTK("S5P_VP_BOT_Y_PTR(0x%08x), S5P_VP_BOT_C_PTR(0x%08x)\n",
				...
				u32 src_off_y)
{
	...
	VPPRINTK("S5P_VP_SRC_H_POSITION(0x%08x), S5P_VP_SRC_V_POSITION(0x%08x)\n",
				...
				u32 dst_off_y)
{
	...
	VPPRINTK("S5P_VP_DST_H_POSITION(0x%08x), S5P_VP_DST_V_POSITION(0x%08x)\n",
				...
				bool ipc_2d)
{
	...
	VPPRINTK("S5P_VP_SRC_WIDTH(%d), S5P_VP_SRC_HEIGHT(%d)\n",
		readl(g_vp_base + S5P_VP_SRC_WIDTH),
		readl(g_vp_base + S5P_VP_SRC_HEIGHT));
	VPPRINTK("S5P_VP_DST_WIDTH(%d), S5P_VP_DST_HEIGHT(%d)\n",
		readl(g_vp_base + S5P_VP_DST_WIDTH),
		readl(g_vp_base + S5P_VP_DST_HEIGHT));
	VPPRINTK("S5P_VP_H_RATIO(0x%08x), S5P_VP_V_RATIO(0x%08x)\n",
...
unsigned short s5p_vp_get_update_status(void)
{
	...
	return readl(g_vp_base + S5P_VP_SHADOW_UPDATE) & S5P_VP_SHADOW_UPDATE_ENABLE;
}
...
void p_s5p_vp_init_layer(u32 top_y_addr,u32 top_c_addr,
                         u32 bottom_y_addr,u32 bottom_c_addr,	
                         u32 s_w,u32 s_h, u32 img_width,u32 img_height)
{
	s5p_vp_init_layer(
            top_y_addr, 
            top_c_addr,
            0,            //bottom_y_addr,
            0,            //bottom_c_addr,
            TVOUT_LITTLE_ENDIAN_MODE, //enum s5p_endian_type src_img_endian,
            s_w,
            s_h,
            0,            //u32 src_off_x,
            0,            //u32 src_x_fract_step,
            0,            //u32 src_off_y,
            s_w,          //u32 src_width,
            s_h,          //u32 src_height,
            0,            //u32 dst_off_x,
            0,            //u32 dst_off_y,
            img_width,    //u32 dst_width,
            img_height,   //u32 dst_height,
            false         //bool ipc_2d
        );
}

void p_s5p_vp_update(void)
{
	s5p_vp_update();
}
